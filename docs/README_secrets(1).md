...
4. Extensión usa `Authorization: Bearer <token>` para `/v3/verify`.

## 3. Rotación de claves (Key Rotation)
### Política
- Mantener siempre 2 claves activas: `KID_CURRENT` y `KID_OLD`.
...
- Clave privada: Nunca se expone. Guardada en KMS (ideal) o variables de entorno (`.env`) en entorno seguro.
- Clave pública: Expuesta únicamente vía `/.well-known/jwks.json`.
